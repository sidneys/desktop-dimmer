 ...
 * @global
 */
let appProductName = packageJson.appProductName || packageJson.name,
...
if (require.main === module) {
    ...
    // npm run-script called with 'debug' parameter
    if (process.argv.slice(3)[0] === 'debug') {
        debugMode = true;
        logger.log('debug', 'enabled');
    }

    // npm run-scripts' name contains ':build'
    if (npmScript.includes(':build')) {
        build();
    }
